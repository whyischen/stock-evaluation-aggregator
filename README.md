...
该命令会根据 `docker-compose.yml` 构建并启动 3 个应用容器，并为 TradingAgents-CN 启动 MongoDB 和 Redis：

- `sea-core`
- `tradingagents-cn-svc`
- `tradingagents-svc`
- `tradingagents-cn-mongodb`
- `tradingagents-cn-redis`
...
| 镜像构建 | 核心服务使用 `Dockerfile.core`，原版插件使用 `Dockerfile.tradingagents`，CN 插件使用 `Dockerfile.tradingagents-cn` |
...
| 数据持久化 | SQLite 数据库挂载到 Docker volume `sea_data`；TradingAgents-CN 的 MongoDB、Redis 和数据缓存使用独立 volume |
| 插件身份 | 两个插件容器通过 `SEA_PLUGIN_SERVICE_ID` 区分加载的 adapter |
| 运行配置 | `.env` 和 Compose `environment` 共同控制 mock、真实调用、超时、权重、LLM Provider 和 A 股数据源 |
...
| `tradingagents-cn-svc` | `sea-plugin-tradingagents-cn:local`，基于 `Dockerfile.tradingagents-cn` 和 `python:3.12-slim` | `8080`，仅 Docker 内部网络 | 封装 GitHub 版 TradingAgents-CN，提供 A 股评估能力 | `SEA_PLUGIN_SERVICE_ID=tradingagents_cn`；默认数据源 `akshare`；依赖 MongoDB + Redis |
| `tradingagents-svc` | `sea-plugin-tradingagents:local`，基于 `Dockerfile.tradingagents` 和 `python:3.12-slim` | `8080`，仅 Docker 内部网络 | 封装 PyPI 版 TradingAgents，提供美股 / 港股评估能力 | `SEA_PLUGIN_SERVICE_ID=tradingagents`；安装 `tradingagents==0.7.0`；默认 Yahoo Finance 数据源 |
| `tradingagents-cn-mongodb` | `docker.m.daocloud.io/library/mongo:7` | 仅 Docker 内部网络 | TradingAgents-CN 新闻、基本面和行情缓存 | 持久化到 Docker volume `tradingagents_cn_mongodb` |
| `tradingagents-cn-redis` | `docker.m.daocloud.io/library/redis:7-alpine` | 仅 Docker 内部网络 | TradingAgents-CN 任务缓存和短期缓存 | 持久化到 Docker volume `tradingagents_cn_redis` |
...
两个插件服务使用同一套 SEA adapter 代码，但使用不同 Dockerfile 和不同上游依赖：

- `tradingagents-cn-svc` 使用 `Dockerfile.tradingagents-cn`，从 GitHub 安装 `hsliuping/TradingAgents-CN`，加载 `SEA_PLUGIN_SERVICE_ID=tradingagents_cn`，服务 A 股 ticker，例如 `600519.SS`。
- `tradingagents-svc` 使用 `Dockerfile.tradingagents`，从 PyPI 安装 `tradingagents==0.7.0`，加载 `SEA_PLUGIN_SERVICE_ID=tradingagents`，服务美股 / 港股 ticker，例如 `AAPL`。
...
- `tradingagents_cn`：A 股，例如 `600519.SS`、`000001.SZ`。上游为 TradingAgents-CN，数据源优先使用 AKShare，可按需配置 Tushare 或 BaoStock，并依赖 MongoDB + Redis。
- `tradingagents`：美股 / 港股，例如 `AAPL`。上游为原版 TradingAgents `0.7.0`，默认使用 Yahoo Finance，无需额外数据库服务。
...
在 Docker 部署中，两个插件镜像已经分别安装对应上游项目，但 `SEA_ENABLE_LIVE_TRADINGAGENTS=false` 仍是默认值，目的是避免误触发 LLM 调用成本。完成 LLM Provider 和 A 股数据源配置后，才建议开启真实调用。
...
### TradingAgents-CN A 股数据源

TradingAgents-CN 容器默认使用 AKShare，免费且不需要 API Key：

```bash
DEFAULT_CHINA_DATA_SOURCE=akshare
CHINA_STOCK_DATA_SOURCE=akshare
AKSHARE_TIMEOUT=30
```

如需使用 Tushare，在 `.env` 中填写 token 并把数据源切换为 `tushare`：

```bash
DEFAULT_CHINA_DATA_SOURCE=tushare
CHINA_STOCK_DATA_SOURCE=tushare
TUSHARE_TOKEN=your_tushare_token
```

BaoStock 可作为无需 token 的备用数据源：

```bash
DEFAULT_CHINA_DATA_SOURCE=baostock
CHINA_STOCK_DATA_SOURCE=baostock
BAOSTOCK_CACHE_DIR=/data/baostock
```

这些变量直接传入 `tradingagents-cn-svc`，SEA 只负责容器配置和请求编排，不解释或改写 TradingAgents-CN 的数据源逻辑。

### TradingAgents-CN MongoDB / Redis

Compose 会为 CN 插件启动独立 MongoDB 和 Redis：

```bash
MONGODB_HOST=tradingagents-cn-mongodb
MONGODB_PORT=27017
MONGODB_DATABASE=tradingagents_cn
MONGODB_URI=mongodb://tradingagents-cn-mongodb:27017/tradingagents_cn
REDIS_HOST=tradingagents-cn-redis
REDIS_PORT=6379
REDIS_DB=0
```

默认服务只在 Docker Compose 内部网络可见，不暴露到宿主机。生产环境如果要使用外部 MongoDB 或 Redis，可以在 `.env` 中覆盖这些连接项，并按需移除或忽略 Compose 内置服务。

### 数据同步

AKShare 和 BaoStock 默认会在运行时按需拉取数据。首次真实评估前建议先启动基础设施并观察 CN 插件日志：

```bash
docker compose up -d tradingagents-cn-mongodb tradingagents-cn-redis
docker compose up -d --build tradingagents-cn-svc
docker compose logs -f tradingagents-cn-svc
```

如果上游 TradingAgents-CN 后续提供独立同步命令，可以在 CN 容器内执行，例如：

```bash
docker compose exec tradingagents-cn-svc python -m tradingagents.dataflows.sync
```

具体同步入口以 TradingAgents-CN 当前版本文档为准；SEA 不内置行情、新闻或基本面同步逻辑。
...
docker compose logs -f tradingagents-cn-mongodb
docker compose logs -f tradingagents-cn-redis
```
...
```bash
docker compose exec tradingagents-svc pip list | grep tradingagents
docker compose exec tradingagents-cn-svc pip list | grep -i tradingagents
```

原版插件应显示 PyPI `tradingagents`，CN 插件应显示从 TradingAgents-CN 安装出的包版本。由于两个上游项目的 Python import 名都可能是 `tradingagents`，判断隔离时以容器镜像和安装来源为准。
...
- `tradingagents-svc` 是否使用 `Dockerfile.tradingagents` 构建，`tradingagents-cn-svc` 是否使用 `Dockerfile.tradingagents-cn` 构建。
...
- A 股请求是否使用 CN 插件，并确认 `DEFAULT_CHINA_DATA_SOURCE`、MongoDB 和 Redis 连接正常。
